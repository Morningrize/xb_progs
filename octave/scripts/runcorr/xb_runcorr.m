...
function [data_corrected, shift_Z, shift_AonZ, ...
          eesZ_fit, errZ_fit, peesAonZ_fit, errAonZ_fit] = xb_runcorr( data, nb_pivots )
    if nargin == 1
        nb_pivots = 30;
    end
    ...
    for pp=pivots
     ...
     [data_corrz, shift_Z] = rrr_corr_inz( data, peesZ_fit, pivots )
     inz = double( [data_corrz.in_Z] );
     ...
     for pp=pivots
         ...
         [inaonz_m, inaonz_i] = xb_multigaus_find( hinaonz, 'triglevel', ...
                                                   max( hinaonz )/10 );
         pees_init = zeros( 3*numel( inaonz_m ), 1 );
         pees_init(1:3:end) = inaonz_m;
         pees_init(2:3:end) = binz( inaonz_im );
     ...
     [data_corrected, shift_AonZ] = rrr_corr_aonz( data_corrz, peesAonZ_fit, pivots );
end
